...
class Log {
    ...
    async log(loggingData, type) {
        ...
        if (type >= this.loglevel || process.env.DEBUG == 'true' || type == 1) {
            ...
            loggingData.name = exports.style.log[this.loglevels[type]](loggingData.name);
    ...
    shutdown() {
        return new Promise((resolve, reject) => {
            this.sentry
                .close(2000)
                .then(async () => {
                await this.log(new Error('Logger successfully shutdown - safe to end all process'), 2);
                resolve();
            })
                .catch(_ => reject(_));
        });
